...
foreach ($folder in $foldersToCopy) {
    ...
    robocopy $folderPath $cloudRoot\src\$folder `
        /IF *.cs *.csproj *.cshtml *.uda *.css *.html *.js package.manifest en-*.xml *.png *.gif *.jpg *.svg *.ico *.woff *.woff2 appsettings.json appsettings.Production.json compilerconfig.json umbraco-cloud.json `
