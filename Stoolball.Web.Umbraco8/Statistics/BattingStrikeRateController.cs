...
{
    public class BattingStrikeRateController : RenderMvcControllerAsync
    {
        ...
        public BattingStrikeRateController(IGlobalSettings globalSettings,
        ...
        public async override Task<ActionResult> Index(ContentModel contentModel)
        {
            ...
            model.Results = (await _statisticsDataSource.ReadBestBattingStrikeRate(model.AppliedFilter).ConfigureAwait(false)).ToList();

            model.AppliedFilter.Paging.PageUrl = Request.Url;
            model.AppliedFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayersWithBattingStrikeRate(model.AppliedFilter).ConfigureAwait(false);

            _statisticsBreadcrumbBuilder.BuildBreadcrumbs(model.Breadcrumbs, model.AppliedFilter);

            model.FilterDescription = _statisticsFilterHumanizer.EntitiesMatchingFilter("Strike rates", _statisticsFilterHumanizer.MatchingUserFilter(model.AppliedFilter));
            model.Metadata.PageTitle = "Best batting strike rate" + _statisticsFilterHumanizer.MatchingFixedFilter(model.AppliedFilter) + _statisticsFilterHumanizer.MatchingUserFilter(model.AppliedFilter);
