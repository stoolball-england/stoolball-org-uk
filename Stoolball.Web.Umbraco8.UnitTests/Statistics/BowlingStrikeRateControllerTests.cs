...
{
    public class BowlingStrikeRateControllerTests : UmbracoBaseTest
    {
        internal static readonly Uri _requestUrl = new Uri("https://example.org" + Constants.Pages.StatisticsUrl + "/bowling-strike-rate?querystring=example");

        public BowlingStrikeRateControllerTests()
        {
            Setup();
        }

        private class TestController : BowlingStrikeRateController
        {
           ...
           : base(
            ...
            {
                return View("BowlingStrikeRate", model);
            }
        }
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            statisticsDataSource.Setup(x => x.ReadBestBowlingStrikeRate(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
