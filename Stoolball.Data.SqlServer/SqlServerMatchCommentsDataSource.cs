...
namespace Stoolball.Data.SqlServer
{
    ...
    public class SqlServerMatchCommentsDataSource : ICommentsDataSource<Match>
    {
        ...
        public async Task<List<HtmlComment>> ReadComments(Guid entityId)
        {
            using (var connection = _databaseConnectionFactory.CreateDatabaseConnection())
            {
                return (await connection.QueryAsync<HtmlComment>($"SELECT CommentId, MemberName, CommentDate, Comment FROM {Tables.Comment} WHERE MatchId = @MatchId ORDER BY CommentDate DESC", new { MatchId = entityId }).ConfigureAwait(false)).ToList();
            }
        }
    }
}
