        ...
        public async Task DeleteCompetition(Competition competition, Guid memberKey, string memberName)
        {
            ...
            using (var connection = _databaseConnectionFactory.CreateDatabaseConnection())
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    await connection.ExecuteAsync($"DELETE FROM {Tables.AwardBy} WHERE SeasonId IN (SELECT SeasonId FROM {Tables.Season} WHERE CompetitionId = @CompetitionId)", new { competition.CompetitionId }, transaction).ConfigureAwait(false);
                    await connection.ExecuteAsync($"DELETE FROM {Tables.AwardedTo} WHERE SeasonId IN (SELECT SeasonId FROM {Tables.Season} WHERE CompetitionId = @CompetitionId)", new { competition.CompetitionId }, transaction).ConfigureAwait(false);
