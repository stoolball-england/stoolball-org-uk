        ...
        public async Task DeleteTeam(Team team, Guid memberKey, string memberName)
        {
            ...
            using (var connection = _databaseConnectionFactory.CreateDatabaseConnection())
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    await connection.ExecuteAsync($"DELETE FROM {Tables.TournamentAward} WHERE PlayerIdentityId IN (SELECT PlayerIdentityId FROM {Tables.PlayerIdentity} WHERE TeamId = @TeamId)", new { auditableTeam.TeamId }, transaction).ConfigureAwait(false);
                    await connection.ExecuteAsync($"DELETE FROM {Tables.SeasonAward} WHERE PlayerIdentityId IN (SELECT PlayerIdentityId FROM {Tables.PlayerIdentity} WHERE TeamId = @TeamId)", new { auditableTeam.TeamId }, transaction).ConfigureAwait(false);
