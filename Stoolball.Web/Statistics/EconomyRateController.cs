...
namespace Stoolball.Web.Statistics
{
    public class EconomyRateController : RenderMvcControllerAsync
    {
        ...
        public async override Task<ActionResult> Index(ContentModel contentModel)
        {
            ...
            model.Results = (await _statisticsDataSource.ReadBestEconomyRate(model.StatisticsFilter).ConfigureAwait(false)).ToList();

            model.StatisticsFilter.Paging.PageUrl = Request.Url;
            model.StatisticsFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayersWithEconomyRate(model.StatisticsFilter).ConfigureAwait(false);

            _statisticsBreadcrumbBuilder.BuildBreadcrumbs(model.Breadcrumbs, model.StatisticsFilter);
            model.Metadata.PageTitle = "Best economy rate" + model.StatisticsFilter.ToString();
