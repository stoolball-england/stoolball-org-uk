...
{
    public class EconomyRateController : RenderMvcControllerAsync
    {
        ...
        public EconomyRateController(IGlobalSettings globalSettings,
        ...
        public async override Task<ActionResult> Index(ContentModel contentModel)
        {
            ...
            model.Results = (await _statisticsDataSource.ReadBestEconomyRate(model.AppliedFilter).ConfigureAwait(false)).ToList();

            model.AppliedFilter.Paging.PageUrl = Request.Url;
            model.AppliedFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayersWithEconomyRate(model.AppliedFilter).ConfigureAwait(false);

            _statisticsBreadcrumbBuilder.BuildBreadcrumbs(model.Breadcrumbs, model.AppliedFilter);
            model.Metadata.PageTitle = "Best economy rate" + _statisticsFilterHumanizer.MatchingFixedFilter(model.AppliedFilter);
