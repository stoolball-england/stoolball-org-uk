...
namespace Stoolball.Web.Statistics
{
    public class BowlingAverageController : RenderMvcControllerAsync
    {
        ...
        public async override Task<ActionResult> Index(ContentModel contentModel)
        {
            ...
            model.Results = (await _statisticsDataSource.ReadBestBowlingAverage(model.StatisticsFilter).ConfigureAwait(false)).ToList();

            model.StatisticsFilter.Paging.PageUrl = Request.Url;
            model.StatisticsFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayersWithBowlingAverage(model.StatisticsFilter).ConfigureAwait(false);

            _statisticsBreadcrumbBuilder.BuildBreadcrumbs(model.Breadcrumbs, model.StatisticsFilter);
            model.Metadata.PageTitle = "Best bowling average" + model.StatisticsFilter.ToString();
