...
{
    public class BowlingAverageController : RenderController, IRenderControllerAsync
    {
        ...
        public async new Task<IActionResult> Index()
        {
            ...
            model.Results = (await _statisticsDataSource.ReadBestBowlingAverage(model.AppliedFilter)).ToList();

            model.AppliedFilter.Paging.PageUrl = new Uri(Request.GetEncodedUrl());
            model.AppliedFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayersWithBowlingAverage(model.AppliedFilter);
            ...
            model.Metadata.PageTitle = "Best bowling average" + _statisticsFilterHumanizer.MatchingFixedFilter(model.AppliedFilter) + _statisticsFilterHumanizer.MatchingUserFilter(model.AppliedFilter);
