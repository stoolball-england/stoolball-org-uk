...
{
    public class PlayerOfTheMatchController : RenderController, IRenderControllerAsync
    {
        ...
        {
            var model = new StatisticsViewModel<PlayerIdentityPerformance>(CurrentPage) { ShowCaption = false };
            ...
            model.Results = (await _statisticsDataSource.ReadPlayerIdentityPerformances(model.AppliedFilter)).ToList();

            model.AppliedFilter.Paging.PageUrl = new Uri(Request.GetEncodedUrl());
            model.AppliedFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayerIdentityPerformances(model.AppliedFilter);

            _statisticsBreadcrumbBuilder.BuildBreadcrumbs(model.Breadcrumbs, model.AppliedFilter);

            model.FilterDescription = _statisticsFilterHumanizer.EntitiesMatchingFilter("Nominations", _statisticsFilterHumanizer.MatchingUserFilter(model.AppliedFilter));
            model.Metadata.PageTitle = "Player of the match nominations" + _statisticsFilterHumanizer.MatchingFixedFilter(model.AppliedFilter) + _statisticsFilterHumanizer.MatchingUserFilter(model.AppliedFilter);
