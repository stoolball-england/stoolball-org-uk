...
namespace Stoolball.Web.Statistics
{
    public class BattingStrikeRateController : RenderMvcControllerAsync
    {
        ...
        public async override Task<ActionResult> Index(ContentModel contentModel)
        {
            ...
            model.Results = (await _statisticsDataSource.ReadBestBattingStrikeRate(model.StatisticsFilter).ConfigureAwait(false)).ToList();

            model.StatisticsFilter.Paging.PageUrl = Request.Url;
            model.StatisticsFilter.Paging.Total = await _statisticsDataSource.ReadTotalPlayersWithBattingStrikeRate(model.StatisticsFilter).ConfigureAwait(false);

            _statisticsBreadcrumbBuilder.BuildBreadcrumbs(model.Breadcrumbs, model.StatisticsFilter);
            model.Metadata.PageTitle = "Best batting strike rate" + model.StatisticsFilter.ToString();
