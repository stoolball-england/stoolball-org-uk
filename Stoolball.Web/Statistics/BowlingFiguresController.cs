...
{
    public class BowlingFiguresController : RenderController, IRenderControllerAsync
    {
        ...
        {
            var model = new StatisticsViewModel<BowlingFigures>(CurrentPage) { ShowCaption = false };
            ...
            model.Results = (await _statisticsDataSource.ReadBowlingFigures(model.AppliedFilter, StatisticsSortOrder.BestFirst)).ToList();

            model.AppliedFilter.Paging.PageUrl = new Uri(Request.GetEncodedUrl());
            model.AppliedFilter.Paging.Total = await _statisticsDataSource.ReadTotalBowlingFigures(model.AppliedFilter);
