...
namespace Stoolball.Data.SqlServer.IntegrationTests.Statistics
{
    ...
    public class ReadMostPlayerInningsTests
    {
        ...
        [Fact]
        public async Task Read_most_player_innings_supports_statistics_filtered_by_team_route()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
            ...
            await ActAndAssertStatistics(filter, dataSource, x => true, i => i.BowlingTeam?.Team?.TeamRoute == _databaseFixture.TestData.TeamWithFullDetails.TeamRoute, i => i.BattingTeam?.Team?.TeamRoute == _databaseFixture.TestData.TeamWithFullDetails.TeamRoute, x => true).ConfigureAwait(false);
        }
        ...
        public async Task Read_most_player_innings_supports_statistics_filtered_by_minimum_score()
        {
            ...
            _queryBuilder.Setup(x => x.BuildWhereClause(It.Is<StatisticsFilter>(x => x.MinimumRunsScored.HasValue))).Returns((" AND RunsScored >= @MinimumRunsScored", new Dictionary<string, object> { { "MinimumRunsScored", filter.MinimumRunsScored } }));
            _queryBuilder.Setup(x => x.BuildWhereClause(It.Is<StatisticsFilter>(x => !x.MinimumRunsScored.HasValue))).Returns((string.Empty, new Dictionary<string, object>()));
            _playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
            var dataSource = new SqlServerBestPlayerTotalStatisticsDataSource(_databaseFixture.ConnectionFactory, _queryBuilder.Object, _playerDataSource.Object);
        ...
        [Fact]
        public async Task Read_most_player_innings_supports_filter_by_team_route()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        public async Task Read_most_player_innings_supports_filter_by_minimum_score()
        {
            ...
            _queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((" AND RunsScored >= @MinimumRunsScored", new Dictionary<string, object> { { "MinimumRunsScored", filter.MinimumRunsScored } }));
            _playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
            var dataSource = new SqlServerBestPlayerTotalStatisticsDataSource(_databaseFixture.ConnectionFactory, _queryBuilder.Object, _playerDataSource.Object);
