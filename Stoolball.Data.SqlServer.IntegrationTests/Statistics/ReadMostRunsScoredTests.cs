...
namespace Stoolball.Data.SqlServer.IntegrationTests.Statistics
{
    ...
    public class ReadMostRunsScoredTests
    {
        ...
        [Fact]
        public async Task Read_total_players_with_runs_scored_supports_filter_by_team_route()
        {
            var filter = new StatisticsFilter { Team = _databaseFixture.TestData.TeamWithFullDetails };
        ...
        public async Task Read_total_players_with_runs_scored_supports_filter_by_minimum_score()
        {
            ...
            _queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((" AND RunsScored >= @MinimumValue", new Dictionary<string, object> { { "MinimumValue", filter.MinimumRunsScored } }));
            var dataSource = new SqlServerBestPlayerTotalStatisticsDataSource(_databaseFixture.ConnectionFactory, _queryBuilder.Object, _playerDataSource.Object);
        ...
        public async Task Read_most_runs_scored_supports_statistics_filtered_by_team_id()
        {
            ...
            await ActAndAssertStatistics(filter, dataSource, x => true, i => i.BowlingTeam?.Team?.TeamId == _databaseFixture.TestData.TeamWithFullDetails.TeamId, i => i.BattingTeam?.Team?.TeamId == _databaseFixture.TestData.TeamWithFullDetails.TeamId!.Value).ConfigureAwait(false);
        }
        ...
        [Fact]
        public async Task Read_most_runs_scored_supports_filter_by_team_route()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
