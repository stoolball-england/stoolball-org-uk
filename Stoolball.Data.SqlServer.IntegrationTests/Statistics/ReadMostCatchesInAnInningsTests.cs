...
namespace Stoolball.Data.SqlServer.IntegrationTests.Statistics
{
    ...
    public class ReadMostCatchesInAnInningsTests
    {
        private readonly SqlServerTestDataFixture _databaseFixture;

        public ReadMostCatchesInAnInningsTests(SqlServerTestDataFixture databaseFixture)
        {
            _databaseFixture = databaseFixture ?? throw new ArgumentNullException(nameof(databaseFixture));
        }

        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_no_filter()
        //{
        ...
        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_filter_by_player_id()
        //{
        ...
        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_filter_by_club_id()
        //{
        ...
        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_filter_by_team_id()
        //{
        ...
        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_filter_by_match_location_id()
        //{
        ...
        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_filter_by_competition_id()
        //{
        ...
        //[Fact]
        //public async Task Read_total_catches_in_an_innings_supports_filter_by_season_id()
        //{
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_returns_player()
        //{
        ...
        //    var results = await dataSource.ReadCatches(filter, StatisticsSortOrder.BestFirst).ConfigureAwait(false);
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_returns_figures()
        //{
        //    var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
        //    var queryBuilder = new Mock<IStatisticsQueryBuilder>();
        //    queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_returns_team()
        //{
        //    var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
        //    var queryBuilder = new Mock<IStatisticsQueryBuilder>();
        //    queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_returns_opposition_team()
        //{
        //    var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
        //    var queryBuilder = new Mock<IStatisticsQueryBuilder>();
        //    queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_returns_match()
        //{
        //    var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
        //    var queryBuilder = new Mock<IStatisticsQueryBuilder>();
        //    queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
        ...
        [Fact]
        public async Task Read_catches_in_an_innings_supports_no_filter()
        {
            var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
            var queryBuilder = new Mock<IStatisticsQueryBuilder>();
            queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
            ...
            var results = await dataSource.ReadCatches(filter).ConfigureAwait(false);

            var expected = _databaseFixture.TestData.Matches
                                .SelectMany(x => x.MatchInnings)
                                .Select(x => x.PlayerInnings.Where(pi => (pi.DismissalType == DismissalType.Caught && pi.DismissedBy != null) ||
                                                                         (pi.DismissalType == DismissalType.CaughtAndBowled && pi.Bowler != null))
                                                            .GroupBy(pi => pi.DismissalType == DismissalType.Caught ? pi.DismissedBy.Player.PlayerId : pi.Bowler.Player.PlayerId,
                                                            x => x, (playerId, catches) => new StatisticsResult<int>
                                                            {
                                                                Player = catches.First().DismissalType == DismissalType.Caught ? catches.First().DismissedBy.Player : catches.First().Bowler.Player,
                                                                Result = catches.Count()
                                                            })
                                )
                                .Where(x => x.Any()).ToList();

            // NEEDS A DIFFERENT QUERY, BUT EVEN THIS DOESN'T MATCH THE DATA SELECTED WITH LINQ - WHY NOT?

            //      SELECT PlayerId, PlayerRoute, TeamId, TeamName, OppositionTeamId AS TeamId, OppositionTeamName AS TeamName,
            //          MatchRoute, MatchStartTime AS StartTime, MatchName, SUM(Catches)
            //          FROM StoolballPlayerInMatchStatistics
            //          GROUP BY PlayerId, PlayerRoute, TeamId, TeamName, OppositionTeamId , OppositionTeamName , MatchRoute, MatchStartTime, MatchName, MatchId, MatchInningsPair
            //          HAVING SUM(Catches) >= 1
            //          ORDER BY SUM(Catches) DESC


            Assert.Equal(expected.Count, results.Count());
            //foreach (var expectedCatches in expected)
            //{
            //    Assert.NotNull(results.SingleOrDefault(x => x.Result.BowlingFiguresId == expectedCatches.BowlingFiguresId));
            //}
        }

        //[Fact]
        //public async Task Read_catches_in_an_innings_supports_filter_by_player_id()
        //{
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_supports_filter_by_club_id()
        //{
        //    var filter = new StatisticsFilter
        //    {
        //        Paging = new Paging
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_supports_filter_by_team_id()
        //{
        //    var filter = new StatisticsFilter
        //    {
        //        Paging = new Paging
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_supports_filter_by_match_location_id()
        //{
        //    var filter = new StatisticsFilter
        //    {
        //        Paging = new Paging
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_supports_filter_by_competition_id()
        //{
        //    var filter = new StatisticsFilter
        //    {
        //        Paging = new Paging
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_supports_filter_by_season_id()
        //{
        //    var filter = new StatisticsFilter
        //    {
        //        Paging = new Paging
        ...
        //[Fact]
        //public async Task Read_best_catches_in_an_innings_sorts_by_most_wickets_for_fewest_runs_with_null_runs_last()
        //{
        //    var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
        //    var queryBuilder = new Mock<IStatisticsQueryBuilder>();
        //    queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
        ...
        //[Fact]
        //public async Task Read_latest_catches_in_an_innings_sorts_by_most_recent_first()
        //{
        //    var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
        //    var queryBuilder = new Mock<IStatisticsQueryBuilder>();
        //    queryBuilder.Setup(x => x.BuildWhereClause(filter)).Returns((string.Empty, new Dictionary<string, object>()));
        ...
        //    var results = await dataSource.ReadCatches(filter, StatisticsSortOrder.LatestFirst).ConfigureAwait(false);
        ...
        //[Fact]
        //public async Task Read_catches_in_an_innings_pages_results()
        //{
