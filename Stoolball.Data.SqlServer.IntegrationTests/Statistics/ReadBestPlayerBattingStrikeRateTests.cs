...
namespace Stoolball.Data.SqlServer.IntegrationTests.Statistics
{
    ...
    public class ReadBestPlayerBattingStrikeRateTests
    {
        ...
        public ReadBestPlayerBattingStrikeRateTests(SqlServerTestDataFixture databaseFixture)
        {
            _databaseFixture = databaseFixture ?? throw new ArgumentNullException(nameof(databaseFixture));
        }

        [Fact]
        public async Task Read_total_players_with_batting_strike_rate_supports_no_filter()
        {
            _queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((string.Empty, new Dictionary<string, object>()));
            var dataSource = new SqlServerBestPlayerAverageStatisticsDataSource(_databaseFixture.ConnectionFactory, _queryBuilder.Object, _playerDataSource.Object);

            var result = await dataSource.ReadTotalPlayersWithBattingStrikeRate(null).ConfigureAwait(false);
        ...
        [Fact]
        public async Task Read_total_players_with_batting_strike_rate_supports_filter_by_team_id()
        {
        ...
        [Fact]
        public async Task Read_total_players_with_batting_strike_rate_supports_filter_by_team_route()
        {
            var filter = new StatisticsFilter { Team = _databaseFixture.TestData.TeamWithFullDetails };
            ...
            var playerInnings = _databaseFixture.TestData.Matches
                ...
                .SelectMany(x => x.MatchInnings)
                .Where(i => i.BattingTeam?.Team?.TeamRoute == _databaseFixture.TestData.TeamWithFullDetails.TeamRoute)
                .SelectMany(x => x.PlayerInnings);
            var expected = playerInnings
                .Where(x => x.RunsScored.HasValue && x.BallsFaced.HasValue)
                .Select(x => x.Batter?.Player?.PlayerId)
        ...
        [Fact]
        public async Task Read_total_players_with_batting_strike_rate_supports_filter_by_match_location_id()
        {
        ...
        [Fact]
        public async Task Read_total_players_with_batting_strike_rate_supports_filter_by_season_id()
        {
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_returns_player()
        {
            ...
            var expected = _databaseFixture.TestData.Players.Where(x => _databaseFixture.TestData.Matches
                                                                            ...
                                                                            .Any(pi => pi.RunsScored.HasValue && pi.BallsFaced.HasValue && pi.BallsFaced.Value > 0));
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_returns_teams()
        {
            var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
            _queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((string.Empty, new Dictionary<string, object>()));
            _playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_returns_statistics()
        {
            var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
            _queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((string.Empty, new Dictionary<string, object>()));
            _playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_club_id()
        {
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_team_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_team_route()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_match_location_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_competition_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_date()
        {
            var dateRangeGenerator = new DateRangeGenerator();
            var (fromDate, untilDate) = dateRangeGenerator.SelectDateRangeToTest(_databaseFixture.TestData.Matches);
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_statistics_filtered_by_season_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        private async Task ActAndAssertStatistics(StatisticsFilter filter, SqlServerBestPlayerAverageStatisticsDataSource dataSource, Func<Stoolball.Matches.Match, bool> matchFilter, Func<MatchInnings, bool> bowlingInningsFilter, Func<MatchInnings, bool> battingInningsFilter, Func<MatchAward, bool> awardsFilter)
        {
            var results = await dataSource.ReadBestBattingStrikeRate(filter).ConfigureAwait(false);
            ...
            var expected = _databaseFixture.TestData.Players.Select(p => new BestStatistic
            {
                ...
                TotalInnings = (int)_databaseFixture.TestData.Matches
                            ...
                            .SelectMany(mi => mi.PlayerInnings)
                            .Count(pi => pi.Batter?.Player?.PlayerId == p.PlayerId && pi.RunsScored.HasValue && pi.BallsFaced.HasValue),
                Average = (_databaseFixture.TestData.Matches
                            .Where(matchFilter)
                            .SelectMany(m => m.MatchInnings)
                            .Where(battingInningsFilter)
                            .SelectMany(mi => mi.PlayerInnings)
                            .Any(pi => pi.Batter?.Player?.PlayerId == p.PlayerId && pi.RunsScored.HasValue && pi.BallsFaced.HasValue && pi.BallsFaced.Value > 0) ?
                                (((decimal)_databaseFixture.TestData.Matches
                                .Where(matchFilter)
                                .SelectMany(m => m.MatchInnings)
                                .Where(battingInningsFilter)
                                .SelectMany(mi => mi.PlayerInnings)
                                .Where(pi => pi.Batter?.Player?.PlayerId == p.PlayerId && pi.RunsScored.HasValue && pi.BallsFaced.HasValue)
            ...
            foreach (var player in expected)
            {
                ...
                {
                    Assert.NotNull(result.Result?.Average);
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_minimum_qualifying_innings()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
            ...
            var expected = _databaseFixture.TestData.Players.Where(x => playerInnings.Any(pi => pi.Batter?.Player?.PlayerId == x.PlayerId && pi.RunsScored.HasValue && pi.BallsFaced.HasValue && pi.BallsFaced > 0) &&
                                                                        playerInnings.Count(pi => pi.Batter?.Player?.PlayerId == x.PlayerId && pi.RunsScored.HasValue && pi.BallsFaced.HasValue) >= filter.MinimumQualifyingInnings);
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_no_filter()
        {
            _queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((string.Empty, new Dictionary<string, object>()));
            _playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
            var dataSource = new SqlServerBestPlayerAverageStatisticsDataSource(_databaseFixture.ConnectionFactory, _queryBuilder.Object, _playerDataSource.Object);

            var results = await dataSource.ReadBestBattingStrikeRate(null).ConfigureAwait(false);
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_filter_by_club_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_filter_by_team_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_filter_by_team_route()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_filter_by_match_location_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
            ...
            var results = await dataSource.ReadBestBattingStrikeRate(It.IsAny<StatisticsFilter>()).ConfigureAwait(false);
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_supports_filter_by_competition_id()
        {
            var filter = new StatisticsFilter
            {
                Paging = new Paging
        ...
        [Fact]
        public async Task Read_best_batting_strike_rate_sorts_by_highest_strike_rate_first()
        {
            var filter = new StatisticsFilter { Paging = new Paging { PageSize = int.MaxValue } };
            _queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((string.Empty, new Dictionary<string, object>()));
            _playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
            ...
            decimal? previousStrikeRate = decimal.MaxValue;
            foreach (var result in results)
            {
                Assert.True(result.Result?.Average <= previousStrikeRate);
                previousStrikeRate = result.Result?.Average;
            }
        }

        [Fact]
        public async Task Read_best_batting_strike_rate_pages_results()
        {
