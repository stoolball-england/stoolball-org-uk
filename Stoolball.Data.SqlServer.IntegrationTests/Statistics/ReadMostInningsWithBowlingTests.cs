...
namespace Stoolball.Data.SqlServer.IntegrationTests.Statistics
{
    ...
    public class ReadMostInningsWithBowlingTests
    {
        ...
        public async Task Read_most_innings_with_bowling_supports_statistics_filtered_by_club_id()
        {
            ...
            await ActAndAssertStatistics(filter, dataSource, x => true, i => i.BattingTeam.Team.TeamId == _databaseFixture.TestData.TeamWithFullDetails.TeamId, i => i.BowlingTeam.Team.TeamId == _databaseFixture.TestData.TeamWithFullDetails.TeamId, x => true).ConfigureAwait(false);
        }
        ...
        public async Task Read_most_innings_with_bowling_supports_statistics_filtered_by_team_id()
        {
            ...
            await ActAndAssertStatistics(filter, dataSource, x => true, i => i.BattingTeam.Team.TeamId == _databaseFixture.TestData.TeamWithFullDetails.TeamId, i => i.BowlingTeam.Team.TeamId == _databaseFixture.TestData.TeamWithFullDetails.TeamId!.Value, x => true).ConfigureAwait(false);
        }
        ...
        public async Task Read_most_innings_with_bowling_supports_statistics_filtered_by_minimum_wickets()
        {
            ...
            var queryBuilder = new Mock<IStatisticsQueryBuilder>();
            queryBuilder.Setup(x => x.BuildWhereClause(It.Is<StatisticsFilter>(x => x.MinimumWicketsTaken.HasValue))).Returns((" AND Wickets >= @MinimumWicketsTaken", new Dictionary<string, object> { { "MinimumWicketsTaken", filter.MinimumWicketsTaken } }));
            queryBuilder.Setup(x => x.BuildWhereClause(It.Is<StatisticsFilter>(x => !x.MinimumWicketsTaken.HasValue))).Returns((string.Empty, new Dictionary<string, object>()));
            var playerDataSource = new Mock<IPlayerDataSource>();
            playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
            var dataSource = new SqlServerBestPlayerTotalStatisticsDataSource(_databaseFixture.ConnectionFactory, queryBuilder.Object, playerDataSource.Object);

            await ActAndAssertStatistics(filter, dataSource, x => true, x => true, x => true, x => x.Wickets >= filter.MinimumWicketsTaken).ConfigureAwait(false);
        }
        ...
        public async Task Read_most_innings_with_bowling_supports_filter_by_minimum_wickets_taken()
        {
            ...
            var queryBuilder = new Mock<IStatisticsQueryBuilder>();
            queryBuilder.Setup(x => x.BuildWhereClause(It.IsAny<StatisticsFilter>())).Returns((" AND Wickets >= @MinimumWicketsTaken", new Dictionary<string, object> { { "MinimumWicketsTaken", filter.MinimumWicketsTaken } }));
            var playerDataSource = new Mock<IPlayerDataSource>();
            playerDataSource.Setup(x => x.ReadPlayers(It.IsAny<PlayerFilter>())).Returns(Task.FromResult(_databaseFixture.TestData.Players));
            var dataSource = new SqlServerBestPlayerTotalStatisticsDataSource(_databaseFixture.ConnectionFactory, queryBuilder.Object, playerDataSource.Object);
