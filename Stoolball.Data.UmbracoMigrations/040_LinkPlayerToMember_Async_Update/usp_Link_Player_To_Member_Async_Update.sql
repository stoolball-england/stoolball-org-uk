...
-- Description:	Checks for outstanding work following linking/unlinking a player to a member, and completes a batch of that work.
--              This is called asynchronously rather than at the time of linking/unlinking the player to avoid a slow update of 
--              StoolballPlayerInMatchStatistics causing SQL timeouts in production.
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[usp_Link_Player_To_Member_Async_Update]
AS
BEGIN
	...
	-- Check for a batch of up to 10 records that need to be updated with a changed PlayerId and PlayerRoute for a PlayerIdentity.
	...
	SELECT TOP 10 PlayerInMatchStatisticsId, p.PlayerId, p.PlayerRoute, s.PlayerRoute AS PlayerRouteToReplace
	...
	WHERE s.PlayerRoute != p.PlayerRoute OR s.PlayerId != p.PlayerId
