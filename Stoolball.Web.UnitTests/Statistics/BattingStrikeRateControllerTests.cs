...
namespace Stoolball.Web.UnitTests.Statistics
{
    public class BattingStrikeRateControllerTests : UmbracoBaseTest
    {
        ...
        public BattingStrikeRateControllerTests() : base()
        {
        }

        private BattingStrikeRateController CreateController()
        {
            return new BattingStrikeRateController(
                Mock.Of<ILogger<BattingStrikeRateController>>(),
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            _bestAverageDataSource.Setup(x => x.ReadBestBattingStrikeRate(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
