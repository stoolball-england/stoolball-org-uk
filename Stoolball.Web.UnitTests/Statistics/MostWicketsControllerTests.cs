...
namespace Stoolball.Web.UnitTests.Statistics
{
    public class MostWicketsControllerTests : UmbracoBaseTest
    {
        ...
        {
            return new MostWicketsController(
                Mock.Of<ILogger<MostWicketsController>>(),
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            _bestTotalDataSource.Setup(x => x.ReadMostWickets(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
