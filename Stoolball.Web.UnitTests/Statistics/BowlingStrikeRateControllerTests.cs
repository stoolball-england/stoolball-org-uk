...
namespace Stoolball.Web.UnitTests.Statistics
{
    public class BowlingStrikeRateControllerTests : UmbracoBaseTest
    {
        ...
        public BowlingStrikeRateControllerTests() : base()
        {
        }

        private BowlingStrikeRateController CreateController()
        {
            return new BowlingStrikeRateController(
                Mock.Of<ILogger<BowlingStrikeRateController>>(),
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            _bestAverageDataSource.Setup(x => x.ReadBestBowlingStrikeRate(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
