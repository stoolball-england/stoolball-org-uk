...
namespace Stoolball.Web.UnitTests.Statistics
{
    public class MostRunOutsControllerTests : UmbracoBaseTest
    {
        ...
        public MostRunOutsControllerTests() : base()
        {
        }

        private MostRunOutsController CreateController()
        {
            return new MostRunOutsController(
                Mock.Of<ILogger<MostRunOutsController>>(),
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            _playerTotalDataSource.Setup(x => x.ReadMostRunOuts(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
