...
namespace Stoolball.Web.UnitTests.Statistics
{
    public class BowlingAverageControllerTests : UmbracoBaseTest
    {
        ...
        {
            return new BowlingAverageController(
                Mock.Of<ILogger<BowlingAverageController>>(),
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            _bestAverageDataSource.Setup(x => x.ReadBestBowlingAverage(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
