...
namespace Stoolball.Web.UnitTests.Statistics
{
    public class BattingAverageControllerTests : UmbracoBaseTest
    {
        ...
        public BattingAverageControllerTests() : base()
        {
        }

        private BattingAverageController CreateController()
        {
            return new BattingAverageController(
                Mock.Of<ILogger<BattingAverageController>>(),
        ...
        public async Task No_results_returns_StatisticsViewModel()
        {
            ...
            _bestAverageDataSource.Setup(x => x.ReadBestBattingAverage(appliedFilter)).Returns(Task.FromResult(results as IEnumerable<StatisticsResult<BestStatistic>>));
