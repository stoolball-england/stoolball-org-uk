...
namespace Stoolball.Web.UnitTests.Account
{
    public class PersonalDetailsSurfaceControllerTests : UmbracoBaseTest
    {
        ...
        public PersonalDetailsSurfaceControllerTests()
        {
            ...
            identity.Setup(x => x.Name).Returns(_currentMember.Object.Name);
            var user = new GenericPrincipal(identity.Object, Array.Empty<string>());
            base.HttpContext.Setup(x => x.User).Returns(user);
            ...
            memberContent.Setup(x => x.Name).Returns(_currentMember.Object.Name);
            memberContent.Setup(x => x.Key).Returns(_currentMember.Object.Key);
            base.MemberService.Setup(x => x.GetByUsername(identity.Object.Name)).Returns(_currentMember.Object);
            base.MemberCache.Setup(x => x.GetByMember(_currentMember.Object)).Returns(memberContent.Object);
            memberContent.Setup(x => x.Id).Returns(_currentMember.Object.Id);
