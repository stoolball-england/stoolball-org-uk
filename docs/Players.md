...
| Has two identities on the owned team, both linked by `Team`. | Has two identities on the owned team, both linked by `Team`. || Moved to Player B. Player A deleted. | Has four identities, all linked by `Team`. `MemberKey` is `NULL`. |
