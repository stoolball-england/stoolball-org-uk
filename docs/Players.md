...
|`Member` | Player with no `MemberKey` and one or more identities linked by `ClubOrTeam`. | Not applicable. | Has a `MemberKey` and all identities are linked by `Member`. | Not applicable. |
