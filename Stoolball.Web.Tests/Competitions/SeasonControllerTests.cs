...
namespace Stoolball.Web.Tests.Competitions
{
    public class SeasonControllerTests : UmbracoBaseTest
    {
        ...
        public async Task Route_not_matching_season_returns_404()
        {
            ...
            dataSource.Setup(x => x.ReadSeasonByRoute(It.IsAny<string>())).Returns(Task.FromResult<Season>(null));
        ...
        {
            var dataSource = new Mock<ISeasonDataSource>();
            dataSource.Setup(x => x.ReadSeasonByRoute(It.IsAny<string>())).ReturnsAsync(new Season { Competition = new Competition { CompetitionName = "Example" } });
